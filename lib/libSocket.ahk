class Socket
{
	...
	__New(Socket:=-1)
	{
		...
		if (!Init)
		{
			...
			VarSetStrCapacity(&WSAData, 394+A_PtrSize)
			if (Error := DllCall("Ws2_32\WSAStartup", "UShort", 0x0202, "Ptr", &WSAData))
				throw ValueError("Error starting Winsock",, Error)
			if (NumGet(WSAData, 2, "UShort") != 0x0202)
				throw ValueError("Winsock version 2.2 not available")
			Init := True
		}
		this.Socket := Socket
	}
	...
	{
		if (this.Socket != -1)
			throw ValueError("Socket already connected")
		...
		throw ValueError("Error connecting")
	}
	...
	Bind(Address)
	{
		...
		throw ValueError("Error binding")
	}
	...
	Accept()
	{
		if ((s := DllCall("Ws2_32\accept", "UInt", this.Socket, "Ptr", 0, "Ptr", 0, "Ptr")) == -1)
			throw ValueError("Error calling accept",, this.GetLastError())
		Sock := Socket(s)
	...
	Disconnect()
	{
		...
		if (DllCall("Ws2_32\closesocket", "UInt", this.Socket, "Int") == -1)
			throw ValueError("Error closing socket",, this.GetLastError())
		this.Socket := -1
		return 1
	}
	...
	MsgSize()
	{
		...
		if (DllCall("Ws2_32\ioctlsocket", "UInt", this.Socket, "UInt", FIONREAD, "UInt*", &argp) == -1)
			throw ValueError("Error calling ioctlsocket",, this.GetLastError())
		return argp
	}
	...
	Send(pBuffer, BufSize, Flags:=0)
	{
		if ((r := DllCall("Ws2_32\send", "UInt", this.Socket, "Ptr", pBuffer, "Int", BufSize, "Int", Flags)) == -1)
			throw ValueError("Error calling send",, this.GetLastError())
		return r
	}
	...
	Recv(&Buffer, BufSize:=0, Flags:=0)
	{
		...
		if ((r := DllCall("Ws2_32\recv", "UInt", this.Socket, "Ptr", &Buffer, "Int", BufSize, "Int", Flags)) == -1)
			throw ValueError("Error calling recv",, this.GetLastError())
		return r
	}
	...
	GetAddrInfo(Address)
	{
		...
		if (Error := DllCall("Ws2_32\getaddrinfo", "AStr", Host, "AStr", Port, "Ptr", &Hints, "Ptr*", Result))
			throw ValueError("Error calling GetAddrInfo",, Error)
		return Result
	}
	...
	AsyncSelect(lEvent)
	{
		if (DllCall("Ws2_32\WSAAsyncSelect"
			...
			throw ValueError("Error calling WSAAsyncSelect",, this.GetLastError())
	}
...
class SocketUDP extends Socket
{
	...
	SetBroadcast(Enable)
	{
		...
		if (DllCall("Ws2_32\setsockopt"
			...
			, "Int", 4) == -1)    ; int    optlen
			throw ValueError("Error calling setsockopt",, this.GetLastError())
	}
}